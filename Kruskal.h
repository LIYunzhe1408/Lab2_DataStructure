...
template <class ElemType, class WeightType>
void MiniSpanTreeKruskal(const AdjListUnDirNetwork<ElemType, WeightType> &g)
// 初始条件：存在网g
// 操作结果：用Kruskal算法构造网g的最小代价生成树
{
    int count, VexNum = g.GetVexNum();
    KruskalEdge<ElemType, WeightType> KEdge;
    MinHeap<KruskalEdge<ElemType, WeightType> > ha(g.GetVexNum());
    ElemType  *kVex, v1, v2;
    ...
    while (count < VexNum - 1)	{
        ...
        v1 = KEdge.vertex1;
        v2 = KEdge.vertex2;
        if (f.Differ(v1, v2))	{	// 边所依附的m两顶点不在同一棵树上
